...
((select id from "USER"
where name = '王小明'),
(select id from "CREDIT_PACKAGE"
where name = '14 堂組合包方案'),
(select credit_amount from "CREDIT_PACKAGE"
where name = '14 堂組合包方案'),
(select price from "CREDIT_PACKAGE"
where name = '14 堂組合包方案')),
((select id from "USER"
where name = '王小明'),
(select id from "CREDIT_PACKAGE"
...
((select id from "COACH" where user_id = (select id from "USER" where email = 'lee2000@hexschooltest.io')), (select id from "SKILL" where name in ('重訓'))),
((select id from "COACH" where user_id = (select id from "USER" where email = 'muscle@hexschooltest.io')), (select id from "SKILL" where name in ('重訓'))),
((select id from "COACH" where user_id = (select id from "USER" where email = 'starplatinum@hexschooltest.io')), (select id from "SKILL" where name in ('重訓'))),
((select id from "COACH" where user_id = (select id from "USER" where name = '肌肉棒子')), (select id from "SKILL" where name in ('瑜伽'))),
((select id from "COACH" where user_id = (select id from "USER" where name = 'Q太郎')), (select id from "SKILL" where name in ('有氧運動'))),
((select id from "COACH" where user_id = (select id from "USER" where name = 'Q太郎')), (select id from "SKILL" where name in ('復健訓練')));
...
((select id from "USER" where name = '李燕容'), (select id from "SKILL" where name in ('重訓')), '重訓基礎課', '2024-11-25 14:00:00', '2024-11-25 16:00:00', 10, 'https://test-meeting.test.io');
...
select "SKILL"."name", Count("SKILL".name) as coach_total from "COACH_LINK_SKILL"
INNER join "SKILL" on "COACH_LINK_SKILL".skill_id = "SKILL".id
Group by "SKILL".name
...
where "CREDIT_PURCHASE".purchase_at < '2024-12-01 00:00:00.000' AND "CREDIT_PURCHASE".purchase_at >= '2024-11-01 00:00:00.000'
...
where purchase_at < '2024-12-01 00:00:00.000' AND purchase_at >= '2024-11-01 00:00:00.000';

-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數

select count(distinct(user_id)) as 預約會員人數 from "COURSE_BOOKING"
where created_at < '2024-12-01 00:00:00.000' AND created_at >= '2024-11-01 00:00:00.000'
and status != '課程已取消';
